...
class Media(TopicRepresentation):
    ...
    representation carrying both. `collage` is the images tiled into a single picture.

    Only the captions are serialized. The media itself is regenerated by re-running the
    representation model, and the collage is written beside the model as a JPEG, so
    persisting either here would bloat the model on disk for no gain.
    """
    ...
    def to_dict(self) -> dict:
        return {"type": "media", "captions": self.captions}
