...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Media(captions=["a cat", "a dog"]), ["a cat", "a dog"]),
    ],
)
...
def test_a_representation_with_no_text_view_has_no_scores():
    ...
    media = Media(summaries={Modality.IMAGE: "collage"})

    assert media.words == []
    assert media.word_scores == []


def test_media_has_no_words_until_it_is_described():
    """A representation may hold a modality that has no text view yet."""
    media = Media(summaries={Modality.IMAGE: "collage"})

    assert media.words == []
    assert str(media) == "No captions available"


def test_described_media_exposes_its_captions_as_words():
    """Once converted, the captions are the representation's text view."""
    media = Media(captions=["a cat", "a dog"])

    assert media.words == ["a cat", "a dog"]


def test_the_collage_is_the_image_summary():
    """A collage stands for a topic's images, as a frame sheet does for video and a montage for audio."""
    media = Media(summaries={Modality.IMAGE: "collage", Modality.AUDIO: b"RIFF"})

    assert media.collage == "collage"
    assert Media().collage is None


def test_media_round_trips_captions_and_paths_but_not_what_is_in_memory():
    """Paths point at media already on disk; media in memory is regenerated by the model."""
    media = Media(
        items={Modality.IMAGE: ["a.png"], Modality.AUDIO: [np.zeros(3)]},
        summaries={Modality.IMAGE: "collage"},
        captions=["a cat"],
    )

    restored = Media.from_dict(media.to_dict())

    assert restored.captions == ["a cat"]
    assert restored.items == {Modality.IMAGE: ["a.png"]}
    assert restored.summaries == {}


def test_media_keeps_each_modality_apart():
    """A topic of photographs and voice notes is one representation carrying both."""
    media = Media(items={Modality.IMAGE: ["a.png"], Modality.AUDIO: ["b.wav"]})

    assert media.images == ["a.png"]
    assert media.items[Modality.AUDIO] == ["b.wav"]
...
def test_a_topic_carries_an_image_representation_alongside_its_keywords():
    ...
    media = Media(summaries={Modality.IMAGE: "collage"}, captions=["a cat"])
    topics.set_data(representations={"Media": {0: media}})

    assert topics[0].representations["Media"].words == ["a cat"]
