...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Media(collage="collage", captions=["a cat", "a dog"]), ["a cat", "a dog"]),
    ],
)
...
def test_described_media_exposes_its_captions_as_words():
    ...
    media = Media(collage="collage", captions=["a cat", "a dog"])

    assert media.words == ["a cat", "a dog"]


def test_media_round_trips_captions_but_not_the_media():
    """Media is regenerated by re-running the model, so only captions are stored."""
    restored = Media.from_dict(Media(collage="collage", captions=["a cat"]).to_dict())

    assert restored.captions == ["a cat"]
    assert restored.collage is None
