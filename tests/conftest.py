...
def base_bertopic():
    model = BERTopic(language="english", verbose=True, min_topic_size=5)
    return model
...
def reduced_topic_model(custom_topic_model, documents):
    ...
    _, _ = model.reduce_topics(documents, nr_topics=12)
    print(set(model.topics_))
    return model
...
def merged_topic_model(custom_topic_model, documents):
    ...
    print("Merged topic model: ", set(model.topics_))
    return model
